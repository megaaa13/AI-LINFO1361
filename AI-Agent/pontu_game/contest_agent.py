...
class MyAgent(AlphaBetaAgent):
	...
	def get_action(self, state, last_action, time_left):
		# Hardcoded first move - contest only
		if state.turns == 0:
			if self.id == 0: return (1, 'SOUTH', 'h', 1, 3)
			else: return (1, 'NORTH', 'h', 1, 1)
	...
	def successors(self, state: pontu_state.PontuState):
		...
		# Return the sorted list of states - contest only
		# if state.turns < 8 and len(states) > 25:
		# 	return states[:25]
		return states
