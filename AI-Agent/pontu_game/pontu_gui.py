...
class GUIState():
    def __init__(self, width=450*2, height=500*2, scale=1, size=5):
        pygame.init()
        self.width = width
        self.height = height
        self.scale = scale
    ...
    def display_state(self,state,display_current_player=True):
        ...
        for i in range(self.size):
            for j in range(self.size):
                self.screen.blit(self.isle, (i * 200, j * 200))

        # Draw the horizontal bridges
        for i in range(self.size - 1):
            for j in range(self.size):
                if state.h_bridges[j][i]:
                    self.screen.blit(self.h_bridge, (i * 200 + 100, j * 200))

        # Draw the vertical bridges
        for i in range(self.size):
            for j in range(self.size - 1):
                if state.v_bridges[j][i]:
                    self.screen.blit(self.v_bridge, (i * 200, j * 200 + 100))

        # Draw the pawns
        for i in range(self.size - 2):
            self.screen.blit(self.green_pawn, (200 * state.cur_pos[0][i][0], 200 * state.cur_pos[0][i][1]))
            self.screen.blit(self.red_pawn, (200 * state.cur_pos[1][i][0], 200 * state.cur_pos[1][i][1]))
        ...
        if display_current_player:
            ...
            textRect1.center = (self.size * 100 - 90, self.size * 200 - 60)
            ...
            textRect2.center = (self.size * 100 + 70, self.size * 200 - 60)
    ...
    def display_winner(self,state):
        ...
        while not quit:
            ...
            textRect.center = ((self.size*2-1)*50, self.size * 200 - 60)
    ...
    def show_timer(self, times_left):
        ...
        textRect = text.get_rect()
        textRect.center = (self.size * 200 - 280, self.size * 200 - 80)
        self.screen.blit(text, textRect)

        font = pygame.font.Font("freesansbold.ttf", 24)
        text = font.render("  " + str(int(times_left[0])) + " sec  ", True, (255, 255, 255), (34, 34, 34))
        textRect = text.get_rect()
        textRect.center = (self.size * 200 - 200, self.size * 200 - 80)
        self.screen.blit(text, textRect)
        ...
        textRect = text.get_rect()
        textRect.center = (self.size * 200 - 280, self.size * 200 - 40)
        self.screen.blit(text, textRect)

        text = font.render("  " + str(int(times_left[1])) + " sec  ", True, (255, 255, 255), (34, 34, 34))
        textRect = text.get_rect()
        textRect.center = (self.size * 200 - 200, self.size * 200 - 40)
        self.screen.blit(text, textRect)
